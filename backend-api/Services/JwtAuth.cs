...
namespace backend_api.Services
{
    public static class JwtAuth
    {
        ...
        public static (int? userId, string? username) Decoder(string token) //Function to decode the information inside token, will decode the token for a username and a id
        {
            ...
            return userIdClaim != null && int.TryParse(userIdClaim.Value, out int userId) ? (userId, usernameClaim) : (null, null); //Condition where the username and id need to be true to return own value, if haven't data return null
        }
